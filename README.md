...
The [`plugin_pose_publisher_in_reference_frame`](doc/plugin_pose_publisher_in_reference_frame.md) documentation explains the frame convention, offset semantics, and output interpretation for the Gazebo system plugin macro that wraps `gz::sim::systems::OdometryPublisher`. Read it before using that macro, because the generated messages use Gazebo odometry message types even when the published pose is not odometry in the semantic sense.
