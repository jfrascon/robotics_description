...
<robot name="plugin_navsat_test" xmlns:xacro="http://www.ros.org/wiki/xacro">
  <xacro:include filename="$(find robotics_description)/urdf/sensors/gnss/generic_macros/plugin_navsat_macro.xacro"/>
  <link name="base_link"/>
  <xacro:plugin_navsat sensor_name="gnss" reference_link="base_link" namespace="robot" topic="gnss/fix"
...
""",
    ...
    expected = {
        ...
        'position_sensing/vertical': ('gaussian_quantized', '11', '12', '13', '14', '15', '16', '17'),
...
<robot name="plugin_navsat_defaults" xmlns:xacro="http://www.ros.org/wiki/xacro">
  <xacro:include filename="$(find robotics_description)/urdf/sensors/gnss/generic_macros/plugin_navsat_macro.xacro"/>
  <link name="base_link"/>
  <xacro:plugin_navsat sensor_name="gnss" reference_link="base_link" topic="gnss/fix"/>
</robot>
""",
    )
...
<robot name="set_props_plugin_navsat_test" xmlns:xacro="http://www.ros.org/wiki/xacro">
  <xacro:include filename="$(find robotics_description)/urdf/sensors/gnss/generic_macros/set_props_plugin_navsat_macro.xacro"/>
  <xacro:property name="noise" value="${dict(type='gaussian', mean=1, stddev=2, bias_mean=3, bias_stddev=4, dynamic_bias_stddev=5, dynamic_bias_correlation_time=6, precision=7)}"/>
  <xacro:property name="cfg" value="${dict(enabled=True, use_gravity=True, always_on=True, update_rate=20, position_horizontal_noise=noise, position_vertical_noise=noise, velocity_horizontal_noise=noise, velocity_vertical_noise=noise, topic='gnss/fix')}"/>
  <xacro:set_props_plugin_navsat prop_prefix="gnss" navsat_sim_cfg="${cfg}"/>
  <result enabled="${gnss_enabled}" topic="${gnss_topic}" noise="${gnss_position_horizontal_noise}"/>
</robot>
""",
    )
